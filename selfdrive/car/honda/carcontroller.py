...
class CarController():
  ...
  def update(self, enabled, CS, frame, actuators,
    ...
    if enabled:
      ...
      hud_car = 0
      self.prev_act_steer = actuators.steer
    ...
    percent_limit = 0.01
    if actuators.steer < (self.prev_act_steer - percent_limit):
      self.prev_act_steer = self.prev_act_steer - percent_limit
    elif actuators.steer > (self.prev_act_steer + percent_limit):
      self.prev_act_steer = self.prev_act_steer + percent_limit
    else:
      self.prev_act_steer = actuators.steer

    # steer torque is converted back to CAN reference (positive when steering right)
    apply_steer = int(interp(-self.prev_act_steer * P.STEER_MAX, P.STEER_LOOKUP_BP, P.STEER_LOOKUP_V))
